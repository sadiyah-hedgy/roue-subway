...
function emailGagnant(p) {
  // Validité d'1 mois uniquement pour les offerts à consommer (cookies, Sub 15).
  const avecValidite = (p.lot_id === "cookies" || p.lot_id === "sub15");
  const blocValidite = avecValidite
    ? `<p style="font-size:14px;line-height:1.5;margin:0;color:${C.vert};font-weight:bold;">⏳ Offre valable 1 mois : à utiliser avant le 25 août 2026.</p>`
    : "";
  const corps = `
    <p style="font-size:15px;line-height:1.6;margin:0 0 16px;">Félicitations 🎉 Tu as gagné :</p>
    <p style="font-size:22px;font-weight:bold;color:${C.vert};margin:0 0 20px;">1 ${p.lot_nom}</p>
